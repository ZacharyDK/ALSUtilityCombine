...
bool UALSMantleComponent::MantleCheck(const FALSMantleTraceSettings& TraceSettings, EDrawDebugTrace::Type DebugType)
{
	...
	bool bPassMontageCheck = true;

	for (UAnimMontage* Mont : ForbiddenMontages)
	{
		if(Mont && OwnerCharacter->GetMainAnimInstance()->Montage_IsPlaying(Mont))
		{
			bPassMontageCheck = false;
			break;
		}

	}

	if(!bPassMontageCheck)
	{
		return false;
	}
