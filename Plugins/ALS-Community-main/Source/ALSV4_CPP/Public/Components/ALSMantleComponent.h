...
DECLARE_DYNAMIC_MULTICAST_DELEGATE(FMantleUpdate);
...
public:
	...
	FMantleUpdate OnMantleStart;

	UPROPERTY(BlueprintAssignable)
	FMantleUpdate OnMantleEnd;
...
protected:
	...
	/*
	If True this line will be called:
	OwnerCharacter->JumpPressedDelegate.AddUniqueDynamic(this, &UALSMantleComponent::OnOwnerJumpInput);
	*/ 
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "ALS|Mantle System")
	bool bBindJumpToOwnerJumpInput = true;

	/*
	Updated everytime MantleCheck() is called
	*/
	UPROPERTY(VisibleAnywhere, BlueprintReadOnly, Category = "ALS|Mantle System")
	FHitResult LastMantleHitResult = FHitResult();

	/*	
	If any of these montages is playing, we can't mantle
	*/
	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, Category = "ALS|Mantle System")
	TArray<UAnimMontage*> ForbiddenMontages = {};
